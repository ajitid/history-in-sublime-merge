...
const viewLineHistory = async (): Promise<void> => {
  if (vscode.window.activeTextEditor) {
    ...
    openSublimeMerge(
      ["search", `file:"${path}" line:${currentLineNumber}-${currentLineNumber}`],
      repository
    );
  }
};
...
export const activate = (context: vscode.ExtensionContext): void => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(`${extensionName}.viewFileHistory`, viewFileHistory)
  );
  context.subscriptions.push(
    vscode.commands.registerCommand(`${extensionName}.viewLineHistory`, viewLineHistory)
  );
  context.subscriptions.push(
